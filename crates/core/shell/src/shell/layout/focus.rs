...
pub(in crate::shell) fn find_focusable_at(node: &WidgetNode, x: f32, y: f32) -> Option<String> {
    find_focusable_at_with_offset(node, x, y, 0.0, 0.0)
}

pub(in crate::shell) fn collect_focus_traversal(node: &WidgetNode) -> Vec<String> {
...
pub(in crate::shell) fn next_focus_target(
    node: &WidgetNode,
    current: Option<&str>,
    backward: bool,
) -> Option<String> {
