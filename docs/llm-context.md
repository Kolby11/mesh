...
`bind:this` is a **live reference**, not a snapshot. All components in one frontend surface share a single Lua realm (one `SurfaceVm` per `FrontendSurfaceComponent`, each component in its own `_ENV`), so the bound variable forwards `__index`/`__newindex` straight to the child's live `_ENV`: reads see current values, calls run synchronously and return real values, and the child's `self.<Event>` channels are exposed so the parent can subscribe (`item_row.Changed:on(fn)`) and receive synchronous fires. Only public members and named event channels cross the boundary — host internals (`self`, `module`, `mesh`, `require`, `__mesh_*`) and lifecycle hooks stay private. The wiring lives in `ScriptContext::install_live_binding` (`crates/core/runtime/scripting/src/context/runtime.rs`) and `FrontendSurfaceComponent::bind_child_instance` (`crates/core/shell/src/shell/component/composition.rs`); cross-surface references stay a marshalled bus.

**Live element references (`refs.<name>`)** are the element-node analog of `bind:this` — the DOM-`querySelector` equivalent for core elements. `refs.<name>` returns a proxy (`crates/core/runtime/scripting/src/context/element_ref.rs`) whose geometry/state fields read from `__mesh_element_metrics`, a per-paint table the shell publishes via `ScriptContext::apply_element_metrics` from `publish_element_metrics` (`interaction_state.rs`). Geometry is inherently last-painted-frame (layout is known only post-paint). Beyond reads, the proxy exposes imperative methods (`focus`, `blur`, `click`, `scroll_into_view`, `scroll_to`, `set_value`) that enqueue `ElementAction`s (each carrying positional `args` plus a DOM-style `options` table); the shell drains them with `apply_element_actions` after each handler and routes through the real interaction paths (`set_focus_target`; `synthesize_click`; the shared `scroll_offsets` map the wheel uses; `input_values`). `refs` resolves a name → live node key via the `ref_node_keys` map built during `collect_element_metrics`. `scroll_into_view` walks the target's scrollable ancestors and nudges each one's offset just enough to reveal the target (CSS "nearest" alignment, deepest-first for nested regions); the geometry is the pure `scroll_into_view_offsets` in `mesh-core-interaction`. `scroll_to(top[, left])` sets a container's own offset; both scroll methods accept `{ smooth = true, duration }` to ease via `advance_scroll_animations` (ticked in `finalize_tree`, `EaseOut`). `refs.input.value` is the live editable text (read + assignable; `set_value`/`__newindex` → `set_value` action); other fields are read-only. Element nodes are Rust-owned `WidgetNode`s, so unlike component `bind:this` this is a Rust-backed proxy, not a shared-VM table reference.
