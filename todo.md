...
- [ ] Reduce retained render hashing and allocation. First passes replaced retained display-list primitive/batch, render-object material/primitive, and retained widget-tree dirty `DefaultHasher` fingerprints with compact deterministic hashers, and removed per-node primitive-slot `Vec` construction during display-list collection. Remaining work is string hashing, repeated attribute parsing, typed dirty fields, stable generation counters, and broader small fixed-array cleanup.
...
- [ ] Improve text ellipsis and clipping measurement. First passes add a bounded 512-entry LRU for ellipsis truncation results and use compact deterministic hashes for text layout/ellipsis cache keys, avoiding repeated binary-search substring measurements and keyed hash overhead for stable labels. Remaining work: compute truncation from shaped glyph advances instead of measuring substrings on first miss.
...
- [ ] Preserve and reuse surface configuration state more aggressively. First passes cache each component runtime's fixed/flexible surface size policy and replace the Wayland surface config fingerprint's keyed hasher with a compact deterministic hasher; remaining work is dirty-bit tracking so unchanged size/title/options skip config construction entirely.
...
- [x] `StyleNodeAttrs::from_node` re-splits class strings per restyle. `WidgetNode` now retains a split class-token cache refreshed by normal frontend node construction and runtime class append paths, and style resolution borrows cached tokens with a stale-cache fallback for direct attribute mutation.
...
- [x] Route slider content paint through the active canvas. Slider rendering now lowers track/active/thumb shapes into painter commands; retained display-list sliders replay those commands through the active `PixelCanvasSession`, and direct slider painting batches the same commands into one backend pass.
- [x] Route icon-font glyph rendering through the glyph atlas. `surface/glyph.rs` now has a canvas-backed icon glyph path with a thread-local Skia image atlas keyed by the glyph cache key, and `draw_icon_resolution_with_axes_in_session` routes font glyphs through `canvas.draw_image_rect` instead of `session.with_buffer`.
...
- [x] Selection-highlight rendering on canvas. `render_display_selection_highlights` now keeps selected retained text on the active `PixelCanvasSession`, drawing base text, highlight fills, and clipped foreground text through canvas/session paths instead of falling back to `session.with_buffer`.
