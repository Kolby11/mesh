...
- [ ] Reduce display-list hashing and allocation. First pass replaced retained display-list primitive/batch `DefaultHasher` signatures with a compact deterministic hasher; remaining work is per-node `Vec` slot construction, string hashing, repeated attribute parsing, typed dirty fields, stable generation counters, and small fixed arrays.
