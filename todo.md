...
- [ ] **TOP PRIORITY — finish promoted nav popover polish and remove manifest surface geometry.** Current `language-popover` / `theme-selector` promotion now renders below the navigation bar, but still needs correct horizontal anchoring under the parent trigger, preserved bubble launch animations, and reliable close/dismiss behavior. Do this in the surface/popup contract rather than ad hoc per-component offsets: placement should come from the popover anchor rect plus CSS-authored layout (`position`, `left/top`, transforms, margins) wherever possible. Avoid markup-level placement nudges such as `offset-y="6"` except for compositor-level constraints that CSS cannot express. Also remove standalone `module.json` surface geometry blocks from tiny embeddable popover components (`anchor`, `layer`, `width`, `height`, `keyboard_mode`, `display_transition`, etc.). The default authoring model should be `<ThemeSelector>` / `<LanguagePopover>` inside another component, with internal/external CSS controlling size and position. `module.json` may expose component variables/settings that the host maps into props/CSS variables, but should not be required to declare Wayland surface defaults for embeddable popovers.
- [ ] **Larger design step — module-declared component variables.** Think through a first-class way for `module.json` to declare configurable variables for frontend components, so modules can expose things like spacing, dimensions, modes, labels, option sets, animation timings, or CSS custom properties without hardcoding host-specific geometry. Those variables should be usable inside the component itself and mappable by a host component when embedding it. This needs a real design pass before implementation: define typing/defaults, prop vs CSS-variable mapping, settings persistence, validation/diagnostics, i18n interactions, and how it differs from Wayland surface layout.
