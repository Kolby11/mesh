...
**Milestone:** v1.17
**Status:** Shipped ✅ (archived 2026-06-07)
...
- [x] **POOL-01**: The shell creates a thread-local Lua VM pool where each pool VM is initialized once with `Lua::sandbox(true)` so stdlib tables are read-only before any component uses it.
- [x] **POOL-02**: `ScriptContext` checks out a pooled VM for each component activation and returns it via a RAII drop guard (`PooledVm`), replacing unconditional `Lua::new()` per component.
- [x] **POOL-03**: The pool grows on-demand with a bounded floor (minimum 4 VMs) so simultaneous surface initialization does not exhaust the pool.
- [x] **POOL-04**: The `PooledVm` RAII guard asserts the same thread identity on drop so cross-thread VM usage is detected at runtime.

### _ENV Isolation

- [x] **ISO-01**: Each checked-out VM slot creates a per-component `_ENV` table where writes are local and reads fall through to the shared read-only pool VM globals via `{ __index = lua.globals() }` metatable.
...
- [x] **ISO-03**: On VM checkin, the component's `env_table` and all registry key handles are dropped via `uninit()` before the slot is returned. Per-component `env_table` isolation provides the state boundary; `Thread::reset()` is not applicable to the main thread in mlua 0.11.
- [x] **ISO-04**: A `pool_baseline_globals` snapshot is captured once at pool VM construction and shared immutably so `sync_state_from_lua()` can distinguish stdlib entries from user-defined reactive component state.
...
- [x] **CACHE-01**: A process-wide source-string cache keyed on FNV64 content hash stores compiled `.luau` source so multi-instance modules parse the source once.
- [x] **CACHE-02**: Pool VM checkout loads the cached source string rather than re-reading from disk on every activation.
- [x] **CACHE-03**: Hot-reload cache eviction is wired — `reload_source()` calls `ChunkCache::remove(fnv64(source))` before `.mesh` recompile; `compile_and_execute()` calls `ChunkCache::get_or_insert(source)` before execution.

### Integration

- [x] **INT-01**: `FrontendSurfaceComponent::create_runtime_for_component` uses `ScriptContext::new_lazy()` and `compile_and_execute()`, replacing the current direct `Lua::new()` constructor call.
- [x] **INT-02**: The shipped navigation bar and audio popover surfaces continue to function correctly after the pool migration — full workspace build passes, scripting and backend test suites pass with zero new regressions.
...
| ISO-01 | Phase 94 | Per-component env_table with __index=globals() metatable |
| ISO-02 | Phase 93 (foundation), Phase 94 (completion) | install_host_api refactor to &Table target |
| ISO-03 | Phase 94 | env_table-based isolation + uninit() on checkin |
...
| INIT-03 | Phase 95 | BackendScriptContext deferred Lua::new() via ensure_lua() |
...
| CACHE-03 | Phase 94 + 95 | ChunkCache::remove in reload_source() for hot-reload eviction |
| INT-01 | Phase 95 | FrontendSurfaceComponent wired to new_lazy() + compile_and_execute |
| INT-02 | Phase 95 | Workspace build + test suite regression proof |
