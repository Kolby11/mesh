...
- [x] 96-01-PLAN.md — Add `state_to_rules` reverse index and `rules_for_state_bit()` to `StyleRuleIndex`
- [x] 96-02-PLAN.md — Replace `state_hash` with `ElementState` in `RetainedNodeSnapshot`, compute `changed_state_bits`
- [x] 96-03-PLAN.md — Fix targeted restyle inheritance and wire `finalize_tree` for narrow interaction restyle
