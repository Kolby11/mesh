# Backlog detail archive — snapshot of 2026-07-28

**This is history, not backlog.** It is a verbatim copy of `docs/BACKLOG.md`
as it stood on 2026-07-28, immediately before that file was split into a terse
open-item list plus this record.

Read it when you need the *detail* behind an item: the dated
`Progress:` / `Added:` / `Completed:` narratives, the benchmark numbers and
gate names, the design rationale, and the reasoning behind approaches that were
tried. The current open-item list lives in
[`docs/BACKLOG.md`](../../docs/BACKLOG.md) and is authoritative for *what is
still open*; where the two disagree about status, the backlog wins.

Nothing below has been edited, including its own stale references
(`.planning/ROADMAP.md` and `.planning/performance/performance-log.md` are now
`.planning/archive/gsd-state/ROADMAP.md` and
`.planning/log/performance-log.md`). Checkbox state is frozen as of the
snapshot date.
...
### Ten-subsystem deep code scan — 2026-07-28

This pass follows the ten subsystem boundaries in
`.planning/performance/sections.md` through their current source paths. It
de-duplicates work already open above (notably narrow invalidation, typed
`WidgetNode`/style declarations, command segmentation, direct SHM paint,
rotation buffers, and the `render_components` phase split). Priorities below
mean P0 correctness/isolation, P1 high-impact hot-path or responsiveness work,
P2 structural simplification, and P3 measured small wins. Every optimization
must retain or add a representative benchmark before landing.

#### 1. Component composition and template evaluation

Trace: compiled templates and imports → embedded runtimes → subtree build →
component memo lookup/store → final annotations and layout.

- [ ] **P1 — Make component memo hits share immutable tree structure.**
      `component/memo.rs::lookup_component_memo` clones the stored
      `WidgetNode`, while `store_component_memo` also clones the completed
      subtree. Introduce an immutable/COW subtree representation or a separate
      build-result plus mutable annotation overlay, so a clean embedded
      component does not deep-clone its entire tree on both cache insertion and
      every hit. Benchmark wide and deeply nested memo-hit surfaces and account
      for retained bytes as well as rebuild time.
- [ ] **P3 — Replace all-runtime descendant-generation scans in memo stores.**
      `store_component_memo` walks every runtime to collect descendant mutation
      generations and allocates a descendant prefix for each store. Maintain a
      parent/child runtime index with an aggregated subtree generation (or
      propagate a dirty generation to ancestors), making validation and store
      cost proportional to the changed branch rather than the surface's total
      runtime count.

#### 2. Retained tree, diff, and display list

Trace: finalized `WidgetNode` identity → retained-node update/dirty roots →
render-object reuse → display-list reconciliation and damage.

- [ ] **P1 — Make `NodeId` collision handling release-safe.**
      `component/runtime_tree.rs::update_retained_node` detects two live nodes
      with the same hash-chained identity only through `debug_assert_ne!`; a
      release build can silently alias their retained snapshots and downstream
      render/display maps. Add checked identity allocation or a collision
      fallback carrying secondary key/path identity, emit diagnostics, and add
      a test hash implementation that forces collisions through the entire
      retained pipeline.
- [ ] **P3 — Reuse retained-update scratch storage with a benchmark gate.**
      `RetainedWidgetTree::update_for_dirty_roots_collect` builds a new
      `update_nodes` `Vec` on each scoped update even though the tree retains
      other frame-to-frame state. Keep a bounded reusable scratch vector (or
      `SmallVec` for typical shallow roots), validate reentrancy/early returns,
      and retain it only if allocation and end-to-end frame benchmarks improve.

#### 3. Style system and theming

Trace: stylesheet compilation/indexing → theme/prop resolver construction →
candidate matching/cascade → computed styles → runtime diagnostic invalidation.

- [ ] **P1 — Remove the diagnostic-only full-tree fingerprint pass.**
      `component/rendering.rs::runtime_style_diagnostic_tree_fingerprint`
      recursively hashes every node's tag, id, classes, key, module, state, and
      children to gate diagnostic re-resolution. Drive this from compiler,
      stylesheet, prop, theme, and retained dirty generations instead; evaluate
      only changed declaration/selector inputs and affected nodes. Measure the
      diagnostic-enabled rebuild separately so instrumentation cannot hide the
      cost.
- [ ] **P3 — Stop cloning the complete CSS-prop map per restyle.**
      `rendering.rs` constructs `StyleResolver::new(theme).with_props(
      surface_css_props.clone())`, while the resolver owns the map. Borrow an
      immutable map, store it behind `Arc`, or cache the resolver by
      prop/theme generation so stable frames do not copy every property.
- [ ] **P3 — Replace flush-all style caches with bounded eviction.**
      Thread-local inline-style, shared-theme, and declaration caches in
      `elements/src/style/resolve.rs` call `clear()` at their caps, causing
      periodic cold-cache latency cliffs. Use a small LRU/clock policy, expose
      hit/eviction counters to profiling, and compare p95 frame time rather than
      only steady-state throughput.

#### 4. Rendering and paint

Trace: render-object lowering → display-list construction → command selection
by damage → Skia raster → child-surface paint.

- [ ] **P1 — Batch multi-rectangle damage into one raster session.**
      `FrontendSurfaceComponent::paint_pixel_regions` and child-popup painting
      call `paint_selected_pixels` once per damage rectangle. That repeats
      render-session setup, profiling resets, and selected-command traversal
      for the same frame. Add a multi-clip API that clears all damaged regions,
      replays each relevant command once where possible, and aggregates text
      metrics/profiling once. Benchmark one, four, and sixteen disjoint rects.
- [ ] **P2 — Converge the immediate and retained renderers.**
      `frontend/render/src/surface/painter/tree.rs` still contains a parallel
      widget-specific immediate renderer, including the temporary rotation
      branch, beside display-list replay. Route compatibility and parity tests
      through one command builder/replay implementation, migrate any remaining
      unique operations, then delete the duplicate paint logic to prevent
      semantic and clipping drift.
- [ ] **P3 — Evict child display lists individually.**
      `shell_component.rs` clears the entire `child_display_lists` map when its
      64-entry cap is crossed, cold-starting every retained popup. Prefer closed
      or inactive child IDs, then LRU eviction, and record churn/eviction
      counters for popup-heavy workloads.

#### 5. Interaction and input

Trace: platform input → target/hit resolution → event payload and handler
lookup → runtime dispatch → focus/hover/active-state restyle.

- [ ] **P1 — Remove filesystem metadata calls from keyboard events.**
      `input/keyboard.rs::current_keyboard_settings` performs two
      `std::fs::metadata` calls on each press/release even when the JSON cache
      is warm. Feed a versioned keyboard-settings snapshot from the shell's
      existing settings watcher into components, leaving the input hot path
      syscall-free.
- [ ] **P2 — Resolve each keyboard target once.**
      `build_keyboard_event` separately searches for a node and its bounds;
      activation paths repeat node, bounds, and click-handler searches even
      though a fused `find_node_with_bounds_by_key` traversal exists. Return a
      borrowed `ResolvedInputTarget` containing identity, node, transformed
      bounds, and relevant handlers, and reuse it for payload construction,
      accessibility, and dispatch.
- [ ] **P2 — Finish the interaction-state move from strings to `NodeId`.**
      Promote the remaining work currently buried in the completed typed
      attribute progress note: migrate `hovered_path`, `focused_key`,
      `pointer_down_key`, active-slider identity, and keyboard activation maps
      to retained `NodeId`; keep strings only at Lua, diagnostics, and explicit
      ref boundaries. Add keyed-list reorder/removal and nested-component
      identity regressions.

#### 6. Script runtime and Rust↔Lua boundary

Trace: per-surface VM attachment → per-component environment initialization →
source preparation/load → host calls/state synchronization → hot reload.

- [ ] **P0 — Isolate component globals in the shared Lua realm.**
      `ScriptContext::set_global_state` writes to `lua.globals()`, while each
      component environment falls through to that shared table.
      `create_runtime_for_component` uses it for module-specific `this`, so
      creating another component can replace the descriptor observed by an
      already-live component. Split shared immutable builtins from
      context-owned globals, write `this` into the component `_ENV`, audit all
      callers, and add a simultaneous two-module regression covering template
      expressions and Lua handlers.
- [ ] **P2 — Delete or turn `ChunkCache` into a real bounded compile cache.**
      `compile_and_execute*` inserts source text by unchecked FNV-64 key and
      immediately loads the original source; production never reads cached
      entries, while hot reload only evicts known old scripts. The global map
      therefore retains unique generated/template-expanded sources without
      avoiding compilation. Either remove it, or cache reusable compiled
      bytecode behind a collision-checked digest with bounded eviction and
      explicit hot-reload ownership. Measure compile time and retained memory.

#### 7. Events, services, and backends

Trace: provider/backend event → contract validation → subscriber index →
component payload cache/runtime delivery → rebuild scheduling.

- [ ] **P1 — Preserve subscriber-index complexity for service updates.**
      `runtime/service_state.rs::deliver_service_event` still visits every
      component on each `Updated` event to cache payloads outside the indexed
      delivery targets, and membership uses `Vec::contains`. Store canonical
      service payloads once and let new/late runtimes read them lazily, or index
      cache recipients separately; make steady-state delivery proportional to
      subscribers and use an epoch marker/bitset for membership.
- [ ] **P3 — Stop cloning, sorting, and deduplicating delivery targets per
      event.** `service_delivery_targets` clones fallback vectors and normalizes
      them for every dispatch. Keep subscriber lists sorted/unique when the
      index changes, or fill a reusable epoch-marked scratch list, with
      fan-out benchmarks covering sparse and broadcast services.

#### 8. Layout

Trace: retained tree/dirtiness → Taffy node/style synchronization → text
measure context construction → compute → layout write-back.

- [ ] **P1 — Retain text measurement contexts by `NodeId`.**
      `elements/src/layout.rs::update_retained_node_styles` walks the complete
      tree on a layout-dirty frame; `update_text_context` recreates `Arc<str>`
      content and repopulates a fresh text-node map even for clean text.
      Extend `PerSurfaceLayoutState` with retained `TextMeasureData`, update it
      only for inserted/removed/text/style-dirty nodes, and let the Taffy
      measure callback borrow that state.
- [ ] **P2 — Make the retained Taffy mapping authoritative.**
      Incremental layout still rebuilds fresh `node_id_to_taffy` and
      `text_nodes` maps for the full tree before compute/write-back. Complete
      the retained map's handling of fragment/unkeyed nodes, maintain it during
      reconciliation, and write layouts through it so sparse dirty frames do
      not reconstruct whole-tree indexes. Gate with text-heavy, keyed-list,
      removal, and embedded-component correctness tests.

#### 9. Presentation and memory

Trace: logical damage → scale/buffer acquisition → raster target → Wayland
attach/damage/commit → configure/event-queue progress.

- [ ] **P1 — Make surface configure waiting nonblocking.**
      `wayland_surface/backend.rs::wait_for_surface_configure` polls and
      dispatches for up to `SURFACE_CONFIGURE_WAIT_DEADLINE` (500ms) inside
      presentation. One unconfigured surface can therefore stall every surface
      on the shell thread. Retain the pending frame, return a typed not-ready
      result, and wake/retry on configure; track the deadline in scheduler
      state for diagnostics instead of sleeping in `present`.
- [ ] **P2 — Batch Wayland commits and event-queue progress per shell frame.**
      The current per-surface present path dispatches/flushes the queue, so
      multi-surface frames repeat connection work and obstruct the planned
      parallel paint/present phase split. Add a frame transaction or
      `present_many` boundary that prepares and commits all ready surfaces,
      then flushes once while preserving per-surface failure isolation.

#### 10. Shell orchestrator, threading, and startup

Trace: installed graph/discovery → parallel frontend compilation/catalog →
surface registration → frame prepare/layout/paint/present → live graph changes.

- [ ] **P0 — Propagate live frontend catalog changes atomically.**
      `activate_frontend_module` builds a new `Arc<FrontendCatalog>` only for
      the newly mounted surface; existing components retain old snapshots, and
      component-only/widget activation returns without updating any host.
      Deactivation likewise removes a surface without rebinding host catalogs.
      Give `Shell` one versioned catalog snapshot, atomically swap/rebind every
      affected component, and invalidate only affected imports, slots, memos,
      and runtimes. Regress enable/disable of a widget, slot contribution, and
      imported component while the host remains mounted.
- [ ] **P1 — Incrementally rebuild the frontend catalog on graph changes.**
      `activate_frontend_module` calls `FrontendCatalog::from_modules` for the
      complete module set, recompiling unrelated frontends for a one-module
      enable. Cache compiled modules by manifest/source fingerprint, update
      dependency and slot indexes from a graph diff, and expose cold-start
      versus live-change timings.
- [ ] **P2 — Share compiled frontend modules instead of cloning roots.**
      `FrontendCatalog::top_level_surfaces` clones `FrontendCatalogEntry`, whose
      root `CompiledFrontendModule` is owned, so each surface duplicates data
      already held by the catalog. Store/return `Arc<CompiledFrontendModule>`
      (with versioned replacement for hot reload) and pass module IDs or shared
      entries through registration. Record catalog and per-surface retained
      memory before/after.

### Suggested attack order (updated 2026-07-28)

1. Local dev environment fix (xkbcommon/freetype/fontconfig) — restores
   in-crate verification for shell/render changes.
2. Shared-Lua `this` isolation + atomic live frontend catalog propagation —
   close the two P0 cross-component correctness risks found in the ten-system
   scan.
3. Canonical workload profiles + perf HUD with paint flashing (L / v1.21) —
   gates every decision below.
4. Nonblocking Wayland configure + watcher-fed keyboard settings — remove
   shell-thread stalls and input-path filesystem calls.
5. Retained text-measure state + subscriber-proportional service delivery +
   batched multi-rectangle raster — attack the remaining whole-tree/fan-out
   hot paths.
6. Structural-sharing component memo hits + narrow invalidation / typed state
   dependencies (v1.18) + affected-subtree re-eval (v1.27).
7. Runtime style-diagnostic invalidation + typed declarations / typed
   `WidgetNode` (v1.23).
8. Incremental shared frontend catalog + single retained renderer + per-surface
   prepare/paint/present phase split and batched Wayland commits.
9. Direct SHM paint and fractional-scale partial damage, re-tested with upload
   instrumentation (D).
